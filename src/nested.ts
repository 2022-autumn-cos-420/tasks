...
export function findQuestion(
    ...
    const specificIndex = questions.findIndex(
        (question: Question): boolean => question.id === id
    );
    if (specificIndex !== -1) {
        return questions[specificIndex];
    }
    return null;
}
...
export function removeQuestion(questions: Question[], id: number): Question[] {
    const updatedArray = questions.filter(
        (question: Question): boolean => question.id !== id
    );
    return updatedArray;
}
...
export function sumPoints(questions: Question[]): number {
    const pointList = questions.map(
        (question: Question): number => question.points
    );
    const pointSum = pointList.reduce(
        (currentTotal: number, points: number) => currentTotal + points,
        0
    );
    return pointSum;
}
...
export function sumPublishedPoints(questions: Question[]): number {
    ...
    const pointList = publishedQuestions.map(
        (question: Question): number => question.points
    );
...
export function toCSV(questions: Question[]): string {
    ...
    const finalString = "id,name,options,points,published" + "\n" + questionCSV;
    return finalString;
}
...
export function addNewQuestion(
    ...
    const newArray = [...questions, blankquestion];
    return newArray;
}
...
export function changeQuestionTypeById(
    ...
    const newQuestion = { ...questions[matchingIndex], type: newQuestionType };
...
export function duplicateQuestionInArray(
    ...
    const newQuestion = duplicateQuestion(
        newId,
        newQuestionArray[matchingIndex]
    );
    newQuestionArray.splice(matchingIndex + 1, 0, newQuestion);
